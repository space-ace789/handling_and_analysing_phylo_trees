...
tree <- drop.tip(tree, c("Canidae", "Ursidae")) #would ignore this step if not removing outgroups from tree
```
...
speciation_rate <- yule_result$lambda* (tree$Nnode + 1) #Nnode +1 represents the number of species
```
...
number_of_tips <- Nnode(tree) + 1

tip_length_list <- capture.output(for(i in 1:number_of_tips) {
terminal.branch<-which(tree$edge[,2]== i)
terminal_diversity <- c(tree$edge.length[terminal.branch])
cat(terminal_diversity, "\n")
} )

tip_length_list <- as.numeric(tip_length_list)

tips <- as.data.frame(tip_length_list)
tips$species <- tree$tip.label
...
ggplot(tips) +
  geom_col(aes(x = species, y=tip_length_list)) +
    scale_x_discrete(labels = label_wrap(10)) +
  labs(x = "Species", y = "Evolutionary History") +
